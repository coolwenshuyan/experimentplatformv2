package com.coolwen.experimentplatformv2.specification;
...
public class SimpleSpecification<T> implements Specification<T> {
    protected static final Logger logger = LoggerFactory.getLogger(SimpleSpecification.class);
    /**
     * 查询的条件列表，是一组列表
     */
    ...
    private Predicate generatePredicate(Root<T> root, CriteriaBuilder criteriaBuilder, SpecificationOperator op) {
        ...
        if ("=".equalsIgnoreCase(op.getOper())) {
            logger.debug(op.getKey() + "," + op.getValue());
